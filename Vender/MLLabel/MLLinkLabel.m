...
- (void)commonInit
{
    ...
    //默认除了话题和@都检测
    _dataDetectorTypes = MLDataDetectorTypeAttributedLink;
        ...
        [attributedString enumerateAttribute:NSLinkAttributeName inRange:NSMakeRange(0, attributedString.length) options:0 usingBlock:^(id value, NSRange range, BOOL *stop) {
            if (value) {
                ...
                if (linkValue.length > 0) {
                    MLLink *link = [MLLink linkWithType:MLLinkTypeOther value:linkValue range:range];
...
{
    if (self.links.count<=0 || self.text.length == 0
        || self.textContainer.size.width <= 0 || self.textContainer.size.height <= 0)
    {
        return nil;
    }
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    if (self.activeLink) {
        ...
        //告诉外面已经点击了某链接
        if(self.delegate&&[self.delegate respondsToSelector:@selector(didClickLink:linkText:linkLabel:)]){
